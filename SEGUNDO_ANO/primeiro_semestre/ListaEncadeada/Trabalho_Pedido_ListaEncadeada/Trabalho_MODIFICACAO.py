# Autor: Gabriel Morais De Oliveira
class Memoria:
    def __init__(self):
        self.clientes = []
        self.produtos = []
    ...
    def mostrarPedidosMemoriaComDesconto(self):
        if len(self.pedidos) == 0:
           return print("\n--------------------------------------------\nNenhum Pedido Cadastrado Na Memória!\n--------------------------------------------\n")
        ...
        for pedido in self.pedidos:
            ...
            print(f"Cliente: {pedido.cliente.nome}")
            print(f"Itens:  ")
            pedido.mostrarItensComDesconto()
            print("============================================")
            print("============================================\n")
...
class ItemPedido:
    def __init__(self, numeroPedido, produto, quantidade, status, percentualDeDesconto):
        self.numeroPedido = numeroPedido
        self.produto = produto
        self.quantidade = quantidade
        self.status = status
        self.percentualDeDesconto = percentualDeDesconto
...
class Pedido:
    ...
    def mostrarItensComDesconto(self):
        if self.itens.inicio != None:
            atual = self.itens.inicio
            while atual:
                print("\n----------------------------------")
                ...
                print(f"\nValor do Item(com desconto): R${(atual.pedido.produto.valor - (atual.pedido.produto.valor * (atual.pedido.percentualDeDesconto/100)))*atual.pedido.quantidade}")
                atual = atual.proximo
        else:
            print("\n--------------------------------------------\nLista De Itens Vazia!\n--------------------------------------------")
...
while True:
    try:
        ...
        print("10-Mostrar Pedidos Com Desconto")
            ...
            if not memoria.pedidosIsNull() and not memoria.produtosIsNull():
                ...
                if pedido != None:
                    ...
                    if produto != None:
                        ...
                        percentualDeDesconto = int(input("Percentual De Desconto:  "))
                        if not(percentualDeDesconto <= 50 and percentualDeDesconto >= 0):
                            percentualDeDesconto = 0
                        ## Adicionar validação, verificar se já existe um produto com aquele ID no array da memória
                        itemPedido = ItemPedido(codItemPedido, produto, quantidade, status, percentualDeDesconto)
        ...
        if op == 10:
            memoria.mostrarPedidosMemoriaComDesconto()
        ...
        if op == 123:
            ...
            item1 = ItemPedido(1, banana, 5, "A caminho", 10)
            item2 = ItemPedido(2, uva, 2, "Preparando", 20)
            item3 = ItemPedido(3, melancia, 1, "Aguardando Pagamento", 50)
